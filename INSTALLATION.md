...
- Access to your AI coding rules repository
...
3. **Install the Extension**
   ```bash
   code --install-extension ai-rules-sync.vsix
   ```
...
2. **Run**: "AI Rules Sync: Configure AI Rules"
...
3. **Configure Required Settings**
   ```json
   {
     "aiRulesSync.repoUrl": "git@github.com:YourOrg/ai-coding-rules.git"
   }
   ```
...
4. **Configure Optional Settings**
   ```json
   {
     "aiRulesSync.teamNames": ["cloud-infra"],
  "aiRulesSync.syncIntervalMinutes": 60
   }
   ```
...
1. **Check Output Logs**
   ...
   - Select "AI Coding Rules" from the dropdown

2. **Manual Sync**
   - Open Command Palette
   - Run: "AI Rules Sync: Refresh Rules"
...
3. **Configuration Issues**
   - Changes to `aiRulesSync.*` settings take effect immediately; no window reload is required
   - Check the "AI Coding Rules" output channel for detailed logs
