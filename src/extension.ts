/**
 * Main entry point for the AI Rules Sync VS Code extension.
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const logChannel = vscode.window.createOutputChannel('AI Coding Rules', { log: true });

    // Guard: Skip activation when no workspace folder is open
    if (!vscode.workspace.workspaceFolders || vscode.workspace.workspaceFolders.length === 0) {
        logChannel.info('No workspace folder found. AI Rules Sync extension will not activate.');
        return;
    }

    console.log('AI Rules Sync extension is now active!');

    // Initialize the rules manager
    rulesManager = new RulesManager(context, logChannel);

    // Register commands
    const refreshCommand = vscode.commands.registerCommand('aiRulesSync.refreshRules', async () => {
        try {
            await rulesManager?.syncRules();
            vscode.window.showInformationMessage('AI coding rules refreshed successfully');
        } catch (error) {
    ...
    const configureCommand = vscode.commands.registerCommand('aiRulesSync.configure', async () => {
        try {
    ...
    const openSettingsCommand = vscode.commands.registerCommand('aiRulesSync.openSettings', async () => {
        await vscode.commands.executeCommand('workbench.action.openSettings', 'aiRulesSync');
    });

    const showConfigCommand = vscode.commands.registerCommand('aiRulesSync.showConfiguration', async () => {
        try {
