/**
 * Configuration manager for the AI Coding Rules extension.
...
export class ConfigManager {
    ...
    private resolveRulesFolderPath(configuredPath: string): string {
        ...
        if (!workspaceFolder) {
            throw new Error('No workspace folder found. Please open a workspace to use AI coding rules.');
        }
    ...
    public getRequiredConfigurationSteps(): { setting: string; description: string; current: string }[] {
        ...
        if (!config.repoUrl.trim()) {
            steps.push({
                setting: 'aiRulesSync.repoUrl',
                description: 'Git repository URL containing your AI coding rules',
    ...
    public getEffectiveConfiguration(): { setting: string; value: string; isDefault: boolean }[] {
        const rawConfig = vscode.workspace.getConfiguration('aiRulesSync');
