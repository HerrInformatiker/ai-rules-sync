/**
 * Main orchestrator for AI coding rules synchronization.
...
export class RulesManager {
        ...
        try {
            ...
            if (!this.configManager.isConfigured()) {
                ...
                const action = await vscode.window.showWarningMessage(
                    'AI coding rules are not configured. Open settings to configure now?',
                    'Open Settings',
                    'Cancel'
                );
                if (action === 'Open Settings') {
                    await vscode.commands.executeCommand('workbench.action.openSettings', 'aiRulesSync');
                }
    ...
    private async handleSyncError(error: Error, isFirstTime: boolean): Promise<void> {
        if (isFirstTime) {
            ...
            const action = await vscode.window.showErrorMessage(
                `Failed to sync AI coding rules: ${error.message}`,
                { modal: true },
                'Retry',
    ...
    private onConfigChanged(event: vscode.ConfigurationChangeEvent): void {
        if (event.affectsConfiguration('aiRulesSync')) {
            this.logger.info('Configuration changed, applying updates...');

            // Check which specific settings changed
            const changedSettings = [
                'aiRulesSync.repoUrl',
                'aiRulesSync.teamNames',
                'aiRulesSync.branch',
                'aiRulesSync.syncIntervalMinutes',
                'aiRulesSync.cacheDirPath',
                'aiRulesSync.rulesFolderPath'
    ...
    private async offerReconfiguration(errorMessage: string): Promise<void> {
        const action = await vscode.window.showWarningMessage(
            `Failed to access AI coding rules repository: ${errorMessage}`,
            'Open Settings',
            'Work with Local Copy'
        );

        if (action === 'Open Settings') {
            await vscode.commands.executeCommand('workbench.action.openSettings', 'aiRulesSync.repoUrl');
        } else if (action === 'Work with Local Copy') {
    ...
    public async showConfigurationWizard(): Promise<void> {
        try {
            ...
            if (steps.length === 0) {
                vscode.window.showInformationMessage('AI coding rules are already configured!');
                return;
            }

            const action = await vscode.window.showInformationMessage(
                'AI Coding Rules needs to be configured to sync your team\'s rules.',
                { modal: true },
                'Configure Now',
                'Open Settings'
            );
    ...
    private async promptForRepositoryUrl(): Promise<void> {
        const repoUrl = await vscode.window.showInputBox({
            prompt: 'Enter your AI coding rules repository URL',
        ...
        if (repoUrl) {
            ...
            const config = vscode.workspace.getConfiguration('aiRulesSync');
            ...
            try {
                ...
                vscode.window.showInformationMessage('AI coding rules synced successfully!');
            } catch (error) {
                await this.handleSyncError(error as Error, true);
            }
        }
